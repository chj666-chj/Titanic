...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, mlp_ratio=4.0, act_layer=nn.GELU, drop_ration=0.):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, dim, num_heads=8,
        ...
        self.mlp = Mlp(in_features=dim, act_layer=act_layer, drop_ration=drop_ratio)
...
class Transformer(nn.Module):
    def __init__(self,dim,depth=12,num_heads=8, qkv_bias=True, attn_drop_ratio=0.,
                 drop_ratio=0.,drop_path_ratio=0.,init_weights=True):
...
if __name__=='__main__':
    ...
    input=torch.rand([134, 1, 42])  #torch.float32
